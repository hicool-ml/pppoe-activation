# PPPOE 激活系统 - 全新宿主机部署指南

本文档详细说明如何在全新的 Linux 服务器上从零开始部署 PPPOE 激活系统，包括开机自启、数据库初始化和持久化配置。

## 目录

1. [系统要求](#系统要求)
2. [安装 Docker](#安装-docker)
3. [获取源代码](#获取源代码)
4. [构建 Docker 镜像](#构建-docker-镜像)
5. [创建持久化目录](#创建持久化目录)
6. [启动容器（开机自启）](#启动容器开机自启)
7. [初始化数据库配置](#初始化数据库配置)
8. [配置网络模式](#配置网络模式)
9. [验证部署](#验证部署)
10. [持久化配置说明](#持久化配置说明)
11. [开机自启验证](#开机自启验证)
12. [离线部署完整流程](#离线部署完整流程)

## 系统要求

### 硬件要求

- **CPU**：双核及以上
- **内存**：2GB 及以上（推荐 4GB）
- **硬盘**：10GB 及以上可用空间
- **网卡**：至少一个网络接口用于 PPPOE 拨号

### 软件要求

- **操作系统**：Linux（推荐 Ubuntu 20.04+ / Debian 11+）
- **Docker**：20.10 及以上
- **Python**：3.12（容器内，无需手动安装）

## 安装 Docker

### Ubuntu / Debian

```bash
# 1. 更新系统包
sudo apt update
sudo apt upgrade -y

# 2. 安装必要的包
sudo apt install -y \
    apt-transport-https \
    ca-certificates \
    curl \
    gnupg \
    lsb-release

# 3. 添加 Docker 官方 GPG 密钥
curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg

# 4. 设置 Docker 仓库
echo \
  "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/ubuntu \
  $(lsb_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null

# 5. 安装 Docker
sudo apt update
sudo apt install -y docker-ce docker-ce-cli containerd.io docker-compose-plugin

# 6. 启动 Docker 服务
sudo systemctl start docker
sudo systemctl enable docker

# 7. 验证安装
sudo docker --version
```

### 验证 Docker 安装

```bash
# 测试 Docker 是否正常工作
sudo docker run hello-world

# 查看版本信息
sudo docker --version
```

## 获取源代码

### 方法 1：使用 Git（需要网络）

```bash
# 1. 安装 Git
sudo apt install -y git

# 2. 克隆仓库
git clone https://github.com/hicool-ml/pppoe-activation.git

# 3. 进入项目目录
cd pppoe-activation
```

### 方法 2：下载压缩包（需要网络）

```bash
# 1. 下载最新版本
wget https://github.com/hicool-ml/pppoe-activation/archive/refs/heads/main.zip

# 2. 解压
unzip main.zip

# 3. 进入项目目录
cd pppoe-activation-main

# 4. 重命名目录
mv pppoe-activation-main pppoe-activation
cd pppoe-activation
```

### 方法 3：使用本地源代码包（无需网络）

如果您已经有源代码压缩包或备份文件：

```bash
# 1. 解压源代码包
tar -xzf pppoe-activation-source.tar.gz
# 或
unzip pppoe-activation-source.zip

# 2. 进入项目目录
cd pppoe-activation
```

**如何创建源代码包**：

在已有源代码的机器上执行：

```bash
# 打包源代码
tar -czf pppoe-activation-source.tar.gz pppoe-activation/

# 或使用 zip
zip -r pppoe-activation-source.zip pppoe-activation/

# 传输到目标服务器
scp pppoe-activation-source.tar.gz user@target-server:/tmp/
```

## 构建 Docker 镜像

### 方法 1：在本地构建镜像（需要网络）

```bash
# 在项目目录下执行
docker build -t pppoe-activation:latest .

# 查看镜像
docker images | grep pppoe-activation
```

### 方法 2：使用本地镜像文件（无需网络）

如果您已经有一个构建好的镜像文件（`.tar` 格式），可以直接导入使用：

```bash
# 1. 导入本地镜像
docker load -i pppoe-activation-image.tar

# 2. 查看导入的镜像
docker images | grep pppoe-activation

# 3. （可选）重命名镜像
docker tag pppoe-activation:latest pppoe-activation:latest
```

**如何获取本地镜像文件**：

在已经构建好镜像的机器上执行：

```bash
# 导出镜像为 tar 文件
docker save -o pppoe-activation-image.tar pppoe-activation:latest

# 查看文件大小
ls -lh pppoe-activation-image.tar

# 传输到目标服务器（使用 scp、U 盘等方式）
scp pppoe-activation-image.tar user@target-server:/tmp/
```

**优势**：
- ✅ 无需网络连接
- ✅ 部署速度快
- ✅ 适合离线环境
- ✅ 镜像版本可控

## 创建持久化目录

```bash
# 创建持久化目录
mkdir -p logs data instance

# 查看目录结构
ls -la
```

**目录说明**：
- `logs/` - 存放所有拨号日志
- `data/` - 存放运行时数据
- `instance/` - 存放数据库文件

### 验证持久化目录权限

确保当前用户对持久化目录有读写权限（Docker 容器内进程通常以 root 或特定 UID 运行）：

```bash
# 设置宽松权限（生产环境可按需收紧）
chmod -R 777 logs data instance

# 或更安全的方式：让容器内用户匹配宿主机 UID（如果知道）
# 例如容器内应用使用 UID 1000，则：
# chown -R 1000:1000 instance/
```

> 💡 **若容器内应用以非 root 用户运行且无写权限，会导致数据库初始化失败或日志无法写入。**

> 💡 **SELinux 用户注意**：
> 在 CentOS/RHEL/Fedora 上，若启用 SELinux，需在挂载参数后添加 `:z` 或 `:Z` 标签：
> ```bash
> -v ${PWD}/instance:/opt/pppoe-activation/instance:Z
> ```
> Ubuntu 默认使用 AppArmor，通常无需额外配置。

## 启动容器（开机自启）

### 启动容器

```bash
# 启动容器（配置开机自启）
docker run -d --name pppoe-activation \
  --restart=unless-stopped \
  --device=/dev/ppp \
  --cap-add=NET_ADMIN \
  -p 80:80 \
  -p 8081:8081 \
  -p 9999:9999 \
  -v ${PWD}/logs:/opt/pppoe-activation/logs \
  -v ${PWD}/data:/opt/pppoe-activation/data \
  -v ${PWD}/instance:/opt/pppoe-activation/instance \
  --network=host \
  pppoe-activation:latest

# 查看容器状态
docker ps | grep pppoe-activation
```

> ⚠️ **注意：`--restart=unless-stopped` 生效的前提是 `docker` 服务已启用开机自启！**
>
> 如果未执行以下命令，宿主机重启后 Docker 引擎不会启动，容器自然也无法恢复：
> ```bash
> sudo systemctl enable docker
> ```
>
> 请务必在安装 Docker 后执行此操作（已在[安装 Docker](#安装-docker)章节第6步包含）。

### 启动参数说明

| 参数 | 说明 |
|------|------|
| `--name pppoe-activation` | 容器名称 |
| `--restart=unless-stopped` | **开机自启**：宿主机重启后自动启动容器 |
| `--device=/dev/ppp` | **映射 PPP 设备到容器**：pppd 拨号必需 |
| `--cap-add=NET_ADMIN` | **添加网络管理权限**：创建 VLAN 子接口必需 |
| `--cap-add=NET_RAW` | **添加原始套接字权限**：PPPoE 拨号必需 |
| `-p 80:80` | 映射端口 80（用户激活页面） |
| `-p 8081:8081` | 映射端口 8081（管理后台） |
| `-p 9999:9999` | 映射端口 9999（配置管理） |
| `-v ${PWD}/logs:...` | **持久化日志**：日志目录挂载 |
| `-v ${PWD}/data:...` | **持久化数据**：数据目录挂载 |
| `-v ${PWD}/instance:...` | **持久化数据库**：数据库目录挂载 |
| `--network=host` | 使用宿主机网络（提高网络性能） |

> ⚠️ **重要权限说明**：
> - `--device=/dev/ppp`：**必需**，pppd 拨号需要访问 PPP 设备。如果没有此参数，拨号会失败并提示"Cannot find /dev/ppp"或"Couldn't open the /dev/ppp device: Operation not permitted"。
> - `--cap-add=NET_ADMIN`：**必需**，创建 VLAN 子接口需要网络管理权限。如果没有此参数，无法创建 VLAN 子接口。
> - `--cap-add=NET_RAW`：**必需**，PPPoE 拨号需要原始套接字权限。如果没有此参数，拨号会失败。

### 开机自启说明

容器配置了 `--restart=unless-stopped` 策略，这意味着：
- ✅ 宿主机重启后，容器会自动启动
- ✅ Docker 服务重启后，容器会自动启动
- ✅ 容器崩溃后，会自动重启
- ⚠️ 如果容器被手动停止（`docker stop`），则不会自动启动

## 初始化数据库配置

### 访问配置页面

```bash
# 打开浏览器访问
http://服务器IP:9999
```

### 配置步骤

1. **选择网络模式**
   - 普通模式：直接使用物理网卡
   - VLAN 模式：使用 VLAN 子接口

2. **配置网络接口**
   - 普通模式：选择一个或多个物理网卡
   - VLAN 模式：选择基础网卡和 VLAN ID

3. **配置持久化存储路径**
   - 日志目录：`./logs`
   - 数据目录：`./data`
   - 数据库目录：`./instance`

4. **提交配置**
   - 点击"保存配置"按钮
   - 等待配置完成

### 数据库自动初始化

容器启动时会自动检查数据库是否存在：
- 如果数据库不存在，会自动创建并初始化
- 如果数据库已存在，会跳过初始化

数据库文件位置：`instance/database.db`

## 配置网络模式

### 普通模式

**适用场景**：单网卡或多网卡直接拨号

**配置方法**：
1. 在配置页面选择"普通模式"
2. 选择一个或多个物理网卡
3. 保存配置

**示例**：
```
网卡列表：enp7s0, enp3s0, enp4s0
```

### VLAN 模式

**适用场景**：需要通过 VLAN 子接口拨号

**配置方法**：
1. 在配置页面选择"VLAN 模式"
2. 选择基础网卡（如 enp7s0）
3. 输入 VLAN ID

**VLAN ID 支持多种格式**：
- 单个 VLAN：`2000`
- 多个 VLAN（逗号分隔）：`2000,2001,2002`
- VLAN 范围：`2000-2005`
- 混合格式：`2000,2001,2005-2010,2015`

**示例**：
```
基础网卡：enp7s0
VLAN ID：2000,2001,2002,2003,2004,2005
```

## 验证部署

### 检查容器状态

```bash
# 查看容器状态
docker ps | grep pppoe-activation

# 应该看到类似输出
# pppoe-activation   Up   5 minutes ago   (healthy)
```

### 查看容器日志

```bash
# 查看启动日志
docker logs pppoe-activation

# 实时查看日志
docker logs -f pppoe-activation
```

### 检查数据库文件

```bash
# 检查数据库是否创建
ls -lh instance/database.db

# 检查数据库权限
ls -l instance/database.db
```

### 检查持久化目录

```bash
# 检查日志目录
ls -la logs/

# 检查数据目录
ls -la data/

# 检查数据库目录
ls -la instance/
```

### 检查容器挂载详情

```bash
# 查看容器挂载信息
docker inspect pppoe-activation --format '{{ json .Mounts }}' | python3 -m json.tool
```

✅ **正确输出应显示类似**：

```json
[
  {
    "Type": "bind",
    "Source": "/home/user/pppoe-activation/instance",
    "Destination": "/opt/pppoe-activation/instance",
    "Mode": "",
    "RW": true,
    "Propagation": "rprivate"
  },
  {
    "Type": "bind",
    "Source": "/home/user/pppoe-activation/logs",
    "Destination": "/opt/pppoe-activation/logs",
    "Mode": "",
    "RW": true,
    "Propagation": "rprivate"
  },
  {
    "Type": "bind",
    "Source": "/home/user/pppoe-activation/data",
    "Destination": "/opt/pppoe-activation/data",
    "Mode": "",
    "RW": true,
    "Propagation": "rprivate"
  }
]
```

**验证要点**：
- `Type` 应为 `"bind"`（表示绑定挂载，非匿名卷）
- `Source` 应为你的宿主机绝对路径
- `RW` 应为 `true`（可读写）

### 访问服务

```bash
# 用户激活页面
http://服务器IP:80/
http://服务器IP/

# 管理后台页面
http://服务器IP:8081/
http://服务器IP:8081/dashboard

# 配置管理页面
http://服务器IP:9999/
```

### 默认账号

**超级管理员**：
- 用户名：`root`
- 密码：`root123`

**普通管理员**：
- 用户名：`admin`
- 密码：`admin123`

**重要**：首次登录后请立即修改密码！

## 持久化配置说明

### 持久化目录结构

```
pppoe-activation/
├── logs/              # 持久化日志目录
│   └── *.log          # 拨号日志文件
├── data/              # 持久化数据目录
└── instance/          # 持久化数据库目录
    └── database.db     # SQLite 数据库文件
```

### 持久化说明

1. **日志持久化**
   - 所有拨号日志都会保存到宿主机的 `logs/` 目录
   - 即使容器被删除，日志也不会丢失
   - 可以使用 `./backup.sh` 脚本备份所有数据

2. **数据库持久化**
   - 数据库文件保存在宿主机的 `instance/database.db`
   - 所有管理员账号、网络配置、激活日志都存储在数据库中
   - 容器重启后数据不会丢失

3. **数据持久化**
   - 运行时数据保存在宿主机的 `data/` 目录
   - 可以存储临时文件和缓存数据

### 持久化验证

```bash
# 在宿主机上查看持久化文件
ls -la logs/
ls -la instance/database.db
ls -la data/

# 在容器内查看持久化文件
docker exec pppoe-activation ls -la /opt/pppoe-activation/logs/
docker exec pppoe-activation ls -la /opt/pppoe-activation/instance/
docker exec pppoe-activation ls -la /opt/pppoe-activation/data/
```

### 主动持久化验证（推荐执行）

为了确保持久化配置真正生效，建议执行以下验证流程：

#### 1. 修改系统数据

登录管理后台（`http://IP:8081`），修改一个管理员密码或添加一个测试用户。

#### 2. 重启宿主机

```bash
sudo reboot
```

#### 3. 重启后验证

```bash
# 等待系统启动完成后，检查容器是否自动运行
docker ps -f name=pppoe-activation

# 应该看到容器正在运行
# pppoe-activation   Up   1 minutes ago   (healthy)

# 检查数据库文件是否存在
ls -l instance/database.db

# 使用新密码登录管理后台
# http://IP:8081
# ✅ 如果能成功登录，说明持久化配置有效
```

**验证要点**：
- ✅ 容器自动启动：`docker ps` 显示容器正在运行
- ✅ 数据保留：使用新密码能成功登录
- ✅ 数据库文件存在：`instance/database.db` 文件存在且有内容

## 开机自启验证

### 重启宿主机

```bash
# 重启宿主机
sudo reboot
```

### 验证容器自动启动

重启后，执行以下命令验证：

```bash
# 等待系统启动完成后
# 查看容器状态
docker ps | grep pppoe-activation

# 应该看到容器正在运行
# pppoe-activation   Up   1 minutes ago   (healthy)
```

### 验证服务可用性

```bash
# 测试用户激活页面
curl http://localhost:80/

# 测试管理后台
curl http://localhost:8081/

# 测试配置管理页面
curl http://localhost:9999/
```

### 验证数据库持久化

```bash
# 检查数据库文件是否存在
ls -lh instance/database.db

# 检查日志文件是否存在
ls -lt logs/ | head -5
```

## 常见问题

### 1. 容器无法启动

**问题**：容器启动后立即退出

**解决方法**：
```bash
# 查看容器日志
docker logs pppoe-activation

# 检查 /dev/ppp 设备
ls -l /dev/ppp

# 检查容器状态
docker ps -a | grep pppoe-activation
```

### 2. 无法访问 Web 界面

**问题**：浏览器无法访问配置页面

**解决方法**：
```bash
# 检查容器状态
docker ps

# 检查防火墙
sudo ufw status
sudo ufw allow 80/tcp
sudo ufw allow 8081/tcp
sudo ufw allow 9999/tcp
```

### 3. pppd 权限问题（重要）

**问题**：拨号失败，日志显示"Permission denied"或"Operation not permitted"

**原因分析**：
pppd 拨号需要两个关键权限：
1. `/dev/ppp` 设备访问权限
2. 网络管理权限（创建 VLAN 子接口）

**解决方法**：

#### 方法 1：检查容器启动参数

```bash
# 检查容器是否包含必要的参数
docker inspect pppoe-activation --format='{{json .HostConfig}}' | python3 -m json.tool | grep -E "Devices|CapAdd"

# 必须包含以下参数：
# "Devices": ["/dev/ppp"]
# "CapAdd": ["NET_ADMIN"]
```

#### 方法 2：重新启动容器（添加必要参数）

```bash
# 停止并删除旧容器
docker stop pppoe-activation
docker rm pppoe-activation

# 重新启动容器（必须包含 --device=/dev/ppp 和 --cap-add=NET_ADMIN）
docker run -d --name pppoe-activation \
  --restart=unless-stopped \
  --device=/dev/ppp \
  --cap-add=NET_ADMIN \
  -p 80:80 -p 8081:8081 -p 9999:9999 \
  -v ${PWD}/logs:/opt/pppoe-activation/logs \
  -v ${PWD}/data:/opt/pppoe-activation/data \
  -v ${PWD}/instance:/opt/pppoe-activation/instance \
  --network=host \
  pppoe-activation:latest
```

#### 方法 3：检查容器内的 /dev/ppp 设备

```bash
# 检查容器内是否有 /dev/ppp 设备
docker exec pppoe-activation ls -l /dev/ppp

# 应该看到类似输出：
# crw-rw-rw- 1 root root 108, 0 Jan  1 00:00 /dev/ppp
# 如果不存在，说明容器启动时缺少 --device=/dev/ppp 参数
```

#### 方法 4：检查 pppd 权限

```bash
# 在容器内检查 pppd 命令权限
docker exec pppoe-activation which pppd

# 在容器内测试 pppd 是否可用
docker exec pppoe-activation pppd --version

# 如果显示"Permission denied"，说明缺少必要的权限
```

**验证修复**：

```bash
# 1. 查看容器日志，确认没有权限错误
docker logs pppoe-activation | grep -i "permission\|denied"

# 2. 测试拨号功能
# 访问 http://服务器IP:80/ 进行测试拨号

# 3. 查看拨号日志
ls -lt logs/ | head -5
```

### 4. VLAN 子接口创建失败

**问题**：日志显示"RTNETLINK answers: Network is down"

**解决方法**：
```bash
# 检查基础网卡状态
ip link show

# 手动启动物理网卡
sudo ip link set enp7s0 up

# 检查容器是否有 NET_ADMIN 权限
docker inspect pppoe-activation | grep CapAdd

# 必须包含 "NET_ADMIN"
```

### 5. 数据库文件权限问题

**问题**：容器无法访问数据库

**解决方法**：
```bash
# 修改数据库文件权限
chmod 666 instance/database.db

# 重启容器
docker restart pppoe-activation
```

### 5. 容器重启后数据丢失

**问题**：重启后配置丢失

**解决方法**：
```bash
# 检查持久化目录
ls -la logs/ instance/ data/

# 检查容器挂载
docker inspect pppoe-activation | grep Mounts
```

### 6. /dev/ppp 设备不存在

**问题**：宿主机没有 /dev/ppp 设备

**解决方法**：

#### 方法 1：加载 PPP 内核模块

```bash
# 加载 PPP 内核模块
sudo modprobe ppp_async
sudo modprobe ppp_generic

# 验证模块是否加载
lsmod | grep ppp

# 检查 /dev/ppp 是否创建
ls -l /dev/ppp
```

#### 方法 2：手动创建 /dev/ppp 设备

```bash
# 创建 /dev/ppp 设备
sudo mknod /dev/ppp c 108 0

# 设置权限
sudo chmod 666 /dev/ppp

# 验证设备
ls -l /dev/ppp
```

#### 方法 3：设置开机自动加载

```bash
# 创建模块加载配置
echo -e "ppp_async\nppp_generic" | sudo tee /etc/modules-load.d/ppp.conf

# 重启后验证
sudo reboot
# 重启后检查
ls -l /dev/ppp
```

**验证修复**：

```bash
# 1. 确认宿主机有 /dev/ppp 设备
ls -l /dev/ppp

# 2. 确认容器启动时映射了 /dev/ppp
docker inspect pppoe-activation --format='{{json .HostConfig.Devices}}' | python3 -m json.tool

# 3. 确认容器内有 /dev/ppp 设备
docker exec pppoe-activation ls -l /dev/ppp
```

## 维护和管理

### 查看日志

```bash
# 容器日志
docker logs -f pppoe-activation

# 拨号日志
ls -lt logs/

# 实时监控拨号日志
tail -f logs/pppoe_*.log
```

### 重启服务

```bash
# 重启容器
docker restart pppoe-activation

# 查看重启后状态
docker ps | grep pppoe-activation
```

### 备份和恢复

```bash
# 备份
./backup.sh

# 恢复
./restore.sh /tmp/pppoe-backup-20260201_120000
```

详细说明请查看 [`BACKUP_README.md`](BACKUP_README.md:1)。

## 离线部署完整流程

本章节详细说明如何在完全没有网络的环境下部署 PPPOE 激活系统。

### 准备阶段（在有网络的机器上）

#### 1. 准备 Docker 镜像

```bash
# 1. 克隆仓库
git clone https://github.com/hicool-ml/pppoe-activation.git
cd pppoe-activation

# 2. 构建镜像
docker build -t pppoe-activation:latest .

# 3. 导出镜像为 tar 文件
docker save -o pppoe-activation-image.tar pppoe-activation:latest

# 4. 查看文件大小
ls -lh pppoe-activation-image.tar
```

#### 2. 准备源代码包

```bash
# 1. 打包源代码
tar -czf pppoe-activation-source.tar.gz pppoe-activation/

# 2. 查看文件大小
ls -lh pppoe-activation-source.tar.gz
```

#### 3. 准备 Docker 安装包（可选）

如果目标机器无法访问网络，需要准备 Docker 安装包：

```bash
# Ubuntu/Debian
# 下载 Docker DEB 包
wget https://download.docker.com/linux/ubuntu/dists/focal/pool/stable/amd64/docker-ce-cli_24.0.7-1~ubuntu.20.04~focal_amd64.deb
wget https://download.docker.com/linux/ubuntu/dists/focal/pool/stable/amd64/containerd.io_1.6.24-1_amd64.deb
wget https://download.docker.com/linux/ubuntu/dists/focal/pool/stable/amd64/docker-ce_24.0.7-1~ubuntu.20.04~focal_amd64.deb

# 打包所有 DEB 文件
mkdir docker-packages
mv *.deb docker-packages/
tar -czf docker-packages.tar.gz docker-packages/
```

#### 4. 创建完整部署包

```bash
# 创建部署目录
mkdir pppoe-deployment
cd pppoe-deployment

# 复制所有文件
cp ../pppoe-activation-image.tar .
cp ../pppoe-activation-source.tar.gz .
cp ../docker-packages.tar.gz .

# 创建部署脚本
cat > deploy.sh << 'EOF'
#!/bin/bash
set -e

echo "========================================="
echo "PPPOE 激活系统 - 离线部署脚本"
echo "========================================="
echo ""

# 检查是否为 root 用户
if [ "$EUID" -ne 0 ]; then 
    echo "请使用 root 权限运行此脚本"
    exit 1
fi

# 1. 安装 Docker（如果未安装）
if ! command -v docker &> /dev/null; then
    echo "正在安装 Docker..."
    
    # 解压 Docker 安装包
    if [ -f docker-packages.tar.gz ]; then
        tar -xzf docker-packages.tar.gz
        cd docker-packages
        dpkg -i *.deb
        cd ..
        echo "Docker 安装完成！"
    else
        echo "未找到 Docker 安装包，请手动安装 Docker"
        exit 1
    fi
else
    echo "Docker 已安装，跳过安装步骤"
fi

# 2. 启动 Docker 服务
echo "启动 Docker 服务..."
systemctl start docker
systemctl enable docker

# 3. 导入 Docker 镜像
echo "导入 Docker 镜像..."
if [ -f pppoe-activation-image.tar ]; then
    docker load -i pppoe-activation-image.tar
    echo "Docker 镜像导入完成！"
else
    echo "未找到 Docker 镜像文件"
    exit 1
fi

# 4. 解压源代码
echo "解压源代码..."
if [ -f pppoe-activation-source.tar.gz ]; then
    tar -xzf pppoe-activation-source.tar.gz
    cd pppoe-activation
    echo "源代码解压完成！"
else
    echo "未找到源代码文件"
    exit 1
fi

# 5. 创建持久化目录
echo "创建持久化目录..."
mkdir -p logs data instance

# 6. 启动容器
echo "启动容器..."
docker run -d --name pppoe-activation \
  --restart=unless-stopped \
  --device=/dev/ppp \
  --cap-add=NET_ADMIN \
  -p 80:80 \
  -p 8081:8081 \
  -p 9999:9999 \
  -v $(pwd)/logs:/opt/pppoe-activation/logs \
  -v $(pwd)/data:/opt/pppoe-activation/data \
  -v $(pwd)/instance:/opt/pppoe-activation/instance \
  --network=host \
  pppoe-activation:latest

# 7. 等待容器启动
echo "等待容器启动..."
sleep 5

# 8. 检查容器状态
echo "检查容器状态..."
docker ps | grep pppoe-activation

echo ""
echo "========================================="
echo "部署完成！"
echo "========================================="
echo ""
echo "访问地址："
echo "  用户激活页面: http://$(hostname -I | awk '{print $1}'):80/"
echo "  管理后台: http://$(hostname -I | awk '{print $1}'):8081/"
echo "  配置管理: http://$(hostname -I | awk '{print $1}'):9999/"
echo ""
echo "默认账号："
echo "  超级管理员: root / root123"
echo "  普通管理员: admin / admin123"
echo ""
echo "请访问配置管理页面进行网络配置："
echo "  http://$(hostname -I | awk '{print $1}'):9999/"
echo ""
EOF

chmod +x deploy.sh

# 打包所有文件
cd ..
tar -czf pppoe-deployment.tar.gz pppoe-deployment/

echo "部署包创建完成：pppoe-deployment.tar.gz"
```

### 部署阶段（在目标机器上）

#### 方法 1：使用部署脚本（推荐）

```bash
# 1. 传输部署包到目标机器
# 使用 scp、U 盘或其他方式

# 2. 解压部署包
tar -xzf pppoe-deployment.tar.gz
cd pppoe-deployment

# 3. 运行部署脚本
sudo ./deploy.sh

# 4. 等待部署完成
# 脚本会自动完成所有部署步骤
```

#### 方法 2：手动部署

```bash
# 1. 安装 Docker（如果未安装）
sudo dpkg -i docker-packages/*.deb

# 2. 启动 Docker 服务
sudo systemctl start docker
sudo systemctl enable docker

# 3. 导入镜像
docker load -i pppoe-activation-image.tar

# 4. 解压源代码
tar -xzf pppoe-activation-source.tar.gz
cd pppoe-activation

# 5. 创建持久化目录
mkdir -p logs data instance

# 6. 启动容器
docker run -d --name pppoe-activation \
  --restart=unless-stopped \
  --device=/dev/ppp \
  --cap-add=NET_ADMIN \
  -p 80:80 \
  -p 8081:8081 \
  -p 9999:9999 \
  -v $(pwd)/logs:/opt/pppoe-activation/logs \
  -v $(pwd)/data:/opt/pppoe-activation/data \
  -v $(pwd)/instance:/opt/pppoe-activation/instance \
  --network=host \
  pppoe-activation:latest

# 7. 检查容器状态
docker ps | grep pppoe-activation
```

### 验证部署

```bash
# 1. 检查容器状态
docker ps | grep pppoe-activation

# 2. 查看容器日志
docker logs pppoe-activation

# 3. 访问配置页面
# http://服务器IP:9999
```

### 文件传输方式

#### 使用 SCP

```bash
# 在有网络的机器上执行
scp pppoe-deployment.tar.gz user@target-server:/tmp/
```

#### 使用 U 盘

```bash
# 1. 复制到 U 盘
cp pppoe-deployment.tar.gz /media/usb/

# 2. 在目标机器上从 U 盘复制
cp /media/usb/pppoe-deployment.tar.gz /tmp/
```

#### 使用 HTTP 服务器

```bash
# 在有网络的机器上启动 HTTP 服务器
cd /path/to/deployment
python3 -m http.server 8000

# 在目标机器上下载
wget http://source-server-ip:8000/pppoe-deployment.tar.gz
```

### 离线部署优势

- ✅ **无需网络**：完全离线环境部署
- ✅ **快速部署**：无需下载依赖，部署速度快
- ✅ **版本可控**：使用固定版本的镜像和源代码
- ✅ **安全可靠**：避免网络传输中的安全风险
- ✅ **适合内网**：适合内网环境部署

### 离线部署注意事项

1. **系统兼容性**：确保目标机器的操作系统与 Docker 镜像兼容
2. **硬件要求**：确保目标机器满足最低硬件要求
3. **文件完整性**：传输后验证文件完整性（使用 md5sum）
4. **权限问题**：使用 root 权限运行部署脚本
5. **防火墙配置**：确保防火墙允许必要的端口（80、8081、9999）

## 下一步

部署完成后，您可以：

1. 访问用户激活页面进行测试
2. 访问管理后台查看激活日志
3. 根据实际需求调整网络配置
4. 配置定期备份任务
5. 修改默认管理员密码

## 更新日志

- **2026-02-01**：添加离线部署完整流程，支持本地镜像和源代码部署
- **2026-02-01**：添加自动启用网卡功能，修复容器重启后无法启动的问题
- **2026-01-30**：实现正确的并发模型，支持多种 VLAN ID 格式
- **2026-01-26**：Docker 镜像优化，减少镜像大小 75-80%


一键更新镜像并重启容器
# 1. 停止并删除旧容器
sudo docker stop pppoe-activation
sudo docker rm pppoe-activation

# 2. 重新启动容器（添加 --device=/dev/ppp 参数）
sudo docker run -d --name pppoe-activation \
  --restart=unless-stopped \
  --device=/dev/ppp \
  --cap-add=NET_ADMIN \
  --cap-add=NET_RAW \
  -p 80:80 \
  -p 8081:8081 \
  -p 9999:9999 \
  -v $(pwd)/logs:/opt/pppoe-activation/logs \
  -v $(pwd)/data:/opt/pppoe-activation/data \
  -v $(pwd)/instance:/opt/pppoe-activation/instance \
  --network=host \
  scerp/pppoe-activation:latest

# 3. 查看容器状态
sudo docker ps | grep pppoe-activation

# 4. 验证/dev/ppp设备
sudo docker exec pppoe-activation ls -l /dev/ppp
