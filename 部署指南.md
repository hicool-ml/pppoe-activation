...
如果您不想自己构建镜像，可以直接从 Docker Hub 拉取预构建的镜像：

```bash
# 拉取最新镜像
sudo docker pull hicool-ml/pppoe-activation:latest

# 拉取指定版本
sudo docker pull hicool-ml/pppoe-activation:v2.1.0
```

**启动容器（使用 Docker run 命令）**：
...
sudo docker run -d \
  ...
  -v $(pwd)/database.db:/opt/pppoe-activation/database.db \
  -v $(pwd)/instance:/opt/pppoe-activation/instance \
  -e TZ=Asia/Shanghai \
  -e APP_PORT=80 \
  -e ADMIN_PORT=8081 \
  -e NETWORK_INTERFACES="enp3s0 enp4s0 enp5s0 enp6s0" \
  --restart unless-stopped \
  hicool-ml/pppoe-activation:latest
```
...
# 查看容器日志
sudo docker logs -f pppoe-activation

# 停止容器
sudo docker stop pppoe-activation

# 启动容器
sudo docker start pppoe-activation

# 重启容器
sudo docker restart pppoe-activation

# 删除容器
sudo docker rm pppoe-activation

# 查看容器详细信息
sudo docker inspect pppoe-activation
```

### 配置 Docker Hub 认证信息

#### 在 VS Code 环境下配置 Docker Hub 认证

**步骤 1：生成 Docker Hub 访问令牌（Access Token）**

1. 访问 Docker Hub：https://hub.docker.com/
2. 登录您的 Docker Hub 账号
3. 点击右上角的用户名，选择"Account Settings"
4. 在左侧菜单中选择"Security"
5. 找到"Access Tokens"部分，点击"New Access Token"
6. 输入访问令牌的描述（如：VS Code Deployment）
7. 选择访问权限（Read & Write）
8. 点击"Generate"生成令牌
9. 复制生成的访问令牌（注意：令牌只显示一次，请妥善保存）

**步骤 2：在 VS Code 终端中登录 Docker Hub**

打开 VS Code 的集成终端（Ctrl + `），执行以下命令：

```bash
# 使用访问令牌登录
docker login -u <your-dockerhub-username> --password-stdin

# 系统会提示输入密码，粘贴刚才复制的访问令牌
# 或者直接在命令中提供令牌（不推荐，因为令牌会保存在历史记录中）
echo <your-access-token> | docker login -u <your-dockerhub-username> --password-stdin
```

**示例**：
```bash
# 假设您的 Docker Hub 用户名是 hicool-ml
docker login -u hicool-ml --password-stdin
# 系统提示输入密码时，粘贴访问令牌
```
...
```bash
# 推送最新版本
docker push hicool-ml/pppoe-activation:latest

# 推送指定版本
docker push hicool-ml/pppoe-activation:v2.1.0
```
...
#### 在 VS Code 中使用 Docker 插件

如果您安装了 VS Code 的 Docker 插件（如 Docker extension），可以：

1. 打开 VS Code
2. 按 Ctrl + Shift + P 打开命令面板
3. 输入"Docker: Images"查看本地镜像
4. 右键点击镜像，选择"Push"
5. 输入镜像名称（如：hicool-ml/pppoe-activation:latest）
6. 如果需要登录，插件会提示您输入用户名和密码

#### 常见问题

**问题 1：登录失败，提示"unauthorized: incorrect username or password"**
...
**问题 2：推送失败，提示"denied: requested access to the resource is denied"**

**原因**：没有权限推送到指定的仓库

**解决方案**：
- 确认您有权限推送到该仓库
- 确认仓库名称正确（格式：username/repository-name）
- 如果是私有仓库，确保您有访问权限
...
1. **妥善保管访问令牌**：访问令牌等同于密码，请妥善保管
2. **定期轮换令牌**：建议定期更换访问令牌
3. **限制令牌权限**：根据需要选择最小权限
4. **不要在命令中直接提供令牌**：避免令牌保存在命令历史中
5. **使用只读令牌**：如果只需要拉取镜像，使用只读权限的令牌
